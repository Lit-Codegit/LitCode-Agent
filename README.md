...
输入 `/` 会打开命令模糊补全；输入 `@` 会搜索当前工作区的目录和文件。目录候选以 `/` 结尾，选中后继续导航；选择文件后会插入 `@{relative/path}`。提交时 LitCode 将受大小限制的 UTF-8 文本快照附在本轮模型上下文中。引用仍受工作区 real-path 边界保护，敏感文件、二进制文件和越界路径会被拒绝。
