...
工具调用在对话流中默认折叠，标题只显示运行状态、工具名和关键参数。展开后显示有长度上限的运行摘要，不展示完整参数 JSON；模型仍会收到完整工具结果，因此界面摘要不会影响 Agent 的后续判断。长摘要和列表（如 `/queue`、`/inbox`）采用内翻页：每页至多 18 行，`j/k` 或方向键翻页。`apply_patch` 的展开内容是对应的统一 diff，新建文件以“新增视图”（全部 `+` 行）呈现，超大改动退化为行数统计摘要。
