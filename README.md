...
核心 MVP 已可运行：包含模型流式适配器、有轮数上限的 Agent 循环、五个本地工具、SQLite 会话持久化、手动上下文压缩、检查点 rewind/redo/fork、命名只读根和端到端测试。
...
输入 `/` 会打开命令模糊补全；输入 `@` 会搜索当前工作区和显式配置的命名只读根。目录候选以 `/` 结尾，选中后继续导航；选择文件后会插入 `@{relative/path}`。提交时 LitCode 将受大小限制的 UTF-8 文本快照附在本轮模型上下文中。敏感文件、二进制文件和未授权路径会被拒绝。
